...
printf '%s\n\n' '$certificateHostName="'"${HOSTNAME}"'"; $certificateBase64Der="'"${CERT_B64_DER}"'"; $matchingCertificates=gci Cert:\LocalMachine\Root|?{$_ .Subject -match $certificateHostName}; if($matchingCertificates){$matchingCertificates|ft Subject,Thumbprint;$confirmation=Read-Host "Delete the above certificates? Type Y to confirm";if($confirmation -match "^[Yy]$"){$matchingCertificates|ri -Force -EA Stop;Write-Host "Deleted."}else{Write-Host "Aborted."}}else{Write-Host "No matching certificates found in LocalMachine\\Root."}; $tempCertPath=Join-Path ([IO.Path]::GetTempPath()) ($certificateHostName+".crt"); [IO.File]::WriteAllBytes($tempCertPath,[Convert]::FromBase64String($certificateBase64Der)); Import-Certificate -FilePath $tempCertPath -CertStoreLocation Cert:\LocalMachine\Root; if(Test-Path $tempCertPath){rm $tempCertPath -Force -EA SilentlyContinue}; Write-Host "Imported certificate and removed "+$tempCertPath'
...
echo "• The certificate is written to a proper temp file in the Windows temp directory and removed after import using Test-Path + Remove-Item -LiteralPath."
