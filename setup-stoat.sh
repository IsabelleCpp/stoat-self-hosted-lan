...
printf '%s\n\n' '$hostName="'"${HOSTNAME}"'"; $b="'"${CERT_B64_DER}"'"; $matches=Get-ChildItem Cert:\LocalMachine\Root | Where-Object { $_.Subject -like "*$hostName*" -or $_.Subject -like "*CN=$hostName*" }; if ($matches -and $matches.Count -gt 0) { $matches | Format-Table Subject,Thumbprint; $ans=Read-Host "Delete the above certificates? Type Y to confirm"; if ($ans -match "^[Yy]$") { $matches | ForEach-Object { $_ | Remove-Item -Force -ErrorAction Stop }; Write-Host "Deleted matching certificates." } else { Write-Host "Deletion aborted by user." } } else { Write-Host "No matching certificates found in LocalMachine\\Root." }; $tmp = Join-Path ([System.IO.Path]::GetTempPath()) ($hostName + ".crt"); [IO.File]::WriteAllBytes($tmp,[Convert]::FromBase64String($b)); Import-Certificate -FilePath $tmp -CertStoreLocation Cert:\LocalMachine\Root; if (Test-Path -LiteralPath $tmp) { Remove-Item -LiteralPath $tmp -Force -ErrorAction SilentlyContinue }; Write-Host ("Imported certificate and removed " + $tmp)'
